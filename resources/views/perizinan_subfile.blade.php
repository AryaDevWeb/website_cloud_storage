    ...
    <div class="max-w-lg">
        <div class="bg-[#111827] border border-[#1e293b] rounded-lg p-6">
            <div class="flex items-center gap-3 mb-5">
                ...
                <div>
                    <h1 class="text-sm font-medium text-white">{{ $cari_file->file }}</h1>
                    <p class="text-xs text-[#94a3b8]">Status: <span class="{{ $cari_file->izin == 1 ? 'text-green-400' : 'text-yellow-400' }}">{{ $cari_file->izin == 1 ? 'Public' : 'Private' }}</span></p>
                </div>
            </div>
            ...
            <form action="/izin_subfile/{{ $cari_file->id }}" method="POST">
                ...
                <div class="space-y-3 mb-5">
                    <label class="flex items-center gap-3 px-4 py-3 bg-[#0a0f1e] border border-[#1e293b] rounded-lg cursor-pointer hover:border-[#334155] transition-colors">
                        <input type="radio" name="izin" value="0" {{ $cari_file->izin == 0 ? 'checked' : '' }}>
                        <div>
                    ...
                    <label class="flex items-center gap-3 px-4 py-3 bg-[#0a0f1e] border border-[#1e293b] rounded-lg cursor-pointer hover:border-[#334155] transition-colors">
                        <input type="radio" name="izin" value="1" {{ $cari_file->izin == 1 ? 'checked' : '' }}>
                        <div>
