    ...
    <div class="max-w-lg">
        <div class="bg-[#111827] border border-[#1e293b] rounded-lg p-6">
            <div class="flex items-center gap-3 mb-5">
                ...
                <div>
                    <h1 class="text-sm font-medium text-white">{{ $folder_izin->nama_folder }}</h1>
                    <p class="text-xs text-[#94a3b8]">Status: <span class="{{ $folder_izin->permission == 1 ? 'text-green-400' : 'text-yellow-400' }}">{{ $folder_izin->permission == 1 ? 'Public' : 'Private' }}</span></p>
                </div>
            </div>
            ...
            <form action="/perizinan_subfolder/{{ $folder_izin->id }}" method="POST">
                ...
                <div class="space-y-3 mb-5">
                    <label class="flex items-center gap-3 px-4 py-3 bg-[#0a0f1e] border border-[#1e293b] rounded-lg cursor-pointer hover:border-[#334155] transition-colors">
                        <input type="radio" name="izin" value="0" {{ $folder_izin->permission == 0 ? 'checked' : '' }}>
                        <div>
                    ...
                    <label class="flex items-center gap-3 px-4 py-3 bg-[#0a0f1e] border border-[#1e293b] rounded-lg cursor-pointer hover:border-[#334155] transition-colors">
                        <input type="radio" name="izin" value="1" {{ $folder_izin->permission == 1 ? 'checked' : '' }}>
                        <div>
